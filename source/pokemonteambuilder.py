...
filetowrite = "generatedteamsd" + str(dsum1weight) + "d" + str(dsum2weight) + "x5.txt"

for testpoke in range(0, 57):
    #testpoke = testnums[testpokes]
    with open(filetowrite, "a") as file:
        file.write("Now Testing:")
        file.write("\n")
        file.write(usagestats[testpoke][0])
        file.write("\n")
        file.write("\n")
    battleteam = [testpoke]
    ...
    for i in range(len(bestteam)):
        ...
        spreadstats = matchupspreads(pokeid)
        spreads = spreadstats[0]
        pokemoves = movespreads(pokeid)
        bestmon = spreads[0][0]
        bestavmon = None
        bestmonmatchups = -1000
        bestmoveset = None
        for j in range(0,len(spreads)):
            localmon = spreads[j][0]
            #print("Testing:")
            #print(spreads[j][0])
            if localmon[5] not in useditems:
                currentsum = getworstteammatchups(bestteam, localmon, i, pokemoves)[1]
                if currentsum >= bestmonmatchups:
                    if localmon[5] != "Assault_Vest":
                        bestmon = localmon
                    else:
                        bestavmon = localmon
                    bestmonmatchups = currentsum
        print(bestmon)
        print(bestavmon)
        if bestavmon == None:
            allmovespreads = getallmovespreads(pokemoves)
            bestmoveset = allmovespreads[0]
            bestmonmatchups = -1000
            for j in range(0,len(allmovespreads)):
                localmoves = allmovespreads[j]
                #print("Testing:")
                #print(localmoves)
                currentsum = getworstteammatchups(bestteam, bestmon, i, localmoves)[1]
                if currentsum >= bestmonmatchups:
                    bestmoveset = localmoves
                    bestmonmatchups = currentsum
            fullteam.append([bestmon, bestmoveset])
        else:
            allmovespreads = getallmovespreads(pokemoves)
            allavspreads = getallmovespreads(pokemoves, False)
            bestmoveset = allmovespreads[0]
            bestmonmatchups = -1000
            isavbetter = False
            for j in range(0,len(allmovespreads)):
                ...
                if currentsum >= bestmonmatchups:
                    bestmoveset = localmoves
                    bestmonmatchups = currentsum
            for j in range(0,len(allavspreads)):
                localmoves = allavspreads[j]
                #print("Testing:")
                #print(localmoves)
                currentsum = getworstteammatchups(bestteam, bestavmon, i, localmoves)[1]
                if currentsum >= bestmonmatchups:
                    bestmoveset = localmoves
                    bestmonmatchups = currentsum
                    isavbetter = True
            if isavbetter:
                fullteam.append([bestavmon, bestmoveset])
            else:
                fullteam.append([bestmon, bestmoveset])
        useditems.append(fullteam[i][0][5])
        print(bestmoveset)
    
    formattoshowdown(fullteam)
